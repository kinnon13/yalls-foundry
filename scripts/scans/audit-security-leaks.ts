...
for await (const entry of walk(".", {
  includeDirs: false,
  skip: ["node_modules", ".git", "dist", "build", "scripts/audit"],
...
await Deno.writeTextFile(`${AUDIT_DIR}/security-leaks.json`, JSON.stringify(report, null, 2));

console.log(`🔒 Found ${hits.length} potential secret leak(s)`);

if (!FIX || hits.length === 0) {
  if (hits.length > 0) {
    console.log(`⚠️  SECURITY: Review ${hits.length} potential leaks in ${AUDIT_DIR}/security-leaks.json`);
