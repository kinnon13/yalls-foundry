...
export async function rockerReferralCreated(params: {
  ...
  //   * Platform 8% fee: 4% direct to platform + 2% buyer upline (tier-based) + 2% seller upline (tier-based, CAP $100)
  ...
  //   * $100 cap on seller upline (2%) prevents excessive payouts on expensive items
}
...
export async function rockerPayoutTriggered(params: {
  ...
  // Commission types:
  // - platform_direct: 4% direct to platform
  // - platform_buyer_upline: 2% to buyer's upline (tier-based split)
  // - platform_seller_upline: 2% to seller's upline (tier-based split, capped $100)
