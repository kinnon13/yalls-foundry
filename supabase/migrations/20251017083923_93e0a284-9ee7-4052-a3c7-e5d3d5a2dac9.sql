-- PR5f: Same-seller spacing (round-robin) for listings in feed fusion

-- Update feed_fusion_home with seller spacing
...
BEGIN
  ...
  listings_cte AS (
    ...
    ORDER BY seller_rank ASC, score DESC, created_at DESC
...
-- Update feed_fusion_profile with seller spacing
...
BEGIN
  ...
  listings_base AS (
    SELECT 
      ...
      l.id,
      l.created_at,
      extract(epoch from (p_cursor - l.created_at))::numeric as score,
      l.seller_entity_id,
      jsonb_build_object(
        'kind', 'listing',
        'id', l.id,
